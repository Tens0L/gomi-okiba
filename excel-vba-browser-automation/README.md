...
| `ENTER` | Enterキーを押す |
| `SPACE` | スペースキーを押す（チェックボックスのトグルなどに） |
| `TAB` | Tabキーを押す |
| `SHIFTTAB` | Shift+Tabキーを押す（前のフィールドに戻る） |
| `UP` / `DOWN` / `LEFT` / `RIGHT` | 矢印キーを押す。D列に回数を指定可（省略時は1回） |

上記以外のキー組み合わせ（例: Ctrl+A, Alt+F4など）も、専用アクションを追加しなくても
`KEY` アクションでそのまま送れます。SendKeysの修飾キー記法は `+`=Shift, `^`=Ctrl,
`%`=Alt で、組み合わせる場合は `+^{TAB}`（Shift+Ctrl+Tab）のように前に並べます。
